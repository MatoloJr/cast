...
<node>
  <interface name="org.cast.tools.Cast1">
    <method name="ListDevices">
      <arg type="a(sssb)" direction="out" name="devices"/>
    </method>
    <method name="Refresh"/>
    ...
    <method name="GetStatus">
      <arg type="(siss)" direction="out" name="status"/>
    </method>
    <signal name="DevicesChanged"/>
    <signal name="SessionChanged">
      <arg type="(siss)" name="status"/>
    </signal>
  </interface>
</node>`;
...
export class CastService {
    ...
    _normalizeStatus(raw) {
        const state = raw?.[0] ?? 'idle';
        const deviceId = raw?.[1] ?? '';
        const deviceName = raw?.[2] ?? '';
        const error = raw?.[3] ?? '';
        return {state, deviceId, deviceName, error};
    }

    /**
     * Ask systemd/D-Bus to activate the helper if a .service is installed.
     */
    async ensureStarted() {
        if (this._destroyed)
            return false;
    ...
    async listDevices() {
        ...
        return (devices || []).map(([id, name, model, online]) => ({
            id,
            name,
            model,
            online: !!online,
